...
A developer-productivity toolkit that makes JavaScript regex creation, testing, and optimization effortless. Features: (1) Fluent builder API with 20+ chainable methods for composing regex patterns in readable English; (2) Pre-built pattern library of 12 production-tested patterns (email RFC 5322, URLs, IPv4/v6, credit cards, etc.); (3) Human-readable regex explainer that breaks any pattern into annotated components; (4) Anti-pattern detector identifying catastrophic backtracking, ReDoS vulnerabilities, and maintainability issues; (5) Automatic regex optimizer that suggests shorter, faster alternatives; (6) Comprehensive test runner with edge-case reporting. Uses robust input validation on all public functions with clear error messages. Clean modular architecture with JSDoc documentation including @example tags on every exported function.
...
function forge() {
  ...
  var api = {
    ...
    raw: function(pattern) { if (typeof pattern !== "string") throw new TypeError("raw() expects string"); parts.push(pattern); return api; },
    /** Match literal text (auto-escaped). @param {string} text @returns {Object} this */
    literal: function(text) { parts.push(escapeRegex(String(text))); return api; },
    ...
    digits: function(count) { if (typeof count !== "number" || count < 1) throw new RangeError("digits() needs positive number"); parts.push("\\d{" + Math.floor(count) + "}"); return api; },
    ...
    words: function(count) { parts.push("\\w{" + Math.floor(count) + "}"); return api; },
    /** Match single whitespace. @returns {Object} this */
    space: function() { parts.push("\\s"); return api; },
    /** Match one or more whitespace. @returns {Object} this */
    spaces: function() { parts.push("\\s+"); return api; },
    /** Match any character except newline. @returns {Object} this */
    any: function() { parts.push("."); return api; },
    ...
    anyOf: function(characters) { parts.push("[" + escapeCharClass(characters) + "]"); return api; },
    /** Match any character NOT in set. @param {string} characters @returns {Object} this */
    noneOf: function(characters) { parts.push("[^" + escapeCharClass(characters) + "]"); return api; },
    /** Match start of line (^). @returns {Object} this */
    startOfLine: function() { parts.push("^"); return api; },
    /** Match end of line ($). @returns {Object} this */
    endOfLine: function() { parts.push("$"); return api; },
    ...
    repeat: function(min, max) { parts.push(max === undefined ? "{" + min + "}" : "{" + min + "," + max + "}"); return api; },
    /** One or more of previous (+). @returns {Object} this */
    oneOrMore: function() { parts.push("+"); return api; },
    /** Zero or more of previous (*). @returns {Object} this */
    zeroOrMore: function() { parts.push("*"); return api; },
    /** Numbered capturing group. @param {Object|string} content @returns {Object} this */
    capture: function(content) { parts.push("(" + extractInner(content) + ")"); return api; },
    ...
    namedCapture: function(name, content) {
      ...
      parts.push("(?<" + name + ">" + extractInner(content) + ")"); return api;
    },
    /** Non-capturing group (?:...). @param {Object|string} content @returns {Object} this */
    group: function(content) { parts.push("(?:" + extractInner(content) + ")"); return api; },
    ...
    or: function() {
      var alts = [];
      for (var ai = 0; ai < arguments.length; ai++) alts.push(extractInner(arguments[ai]));
      parts.push("(?:" + alts.join("|") + ")"); return api;
    },
    /** Positive lookahead (?=...). @param {Object|string} content @returns {Object} this */
    followedBy: function(content) { parts.push("(?=" + extractInner(content) + ")"); return api; },
    /** Negative lookahead (?!...). @param {Object|string} content @returns {Object} this */
    notFollowedBy: function(content) { parts.push("(?!" + extractInner(content) + ")"); return api; },
    ...
    withFlags: function(flagStr) { flags = String(flagStr); return api; },
...
var phone = forge().startOfLine().optional(forge().literal("(")).digits(3).optional(forge().literal(")")).anyOf(" -.").digits(3).anyOf(" -.").digits(4).endOfLine().build();
console.log("Phone: " + phone + " | test: " + phone.test("(555) 123-4567"));
var semv = forge().startOfLine().namedCapture("major", forge().digit().oneOrMore()).literal(".").namedCapture("minor", forge().digit().oneOrMore()).literal(".").namedCapture("patch", forge().digit().oneOrMore()).endOfLine().build();
console.log("Semver parse: " + JSON.stringify("1.2.3".match(semv).groups));
console.log("Email valid: " + patterns.email().test("user@example.com"));
console.log("Explain: " + explain(/^\d+$/).explanation.join(", "));
var result = detectAntiPatterns(/(a+)+$/);
console.log("Anti-pattern: " + result.warnings[0].severity + " — " + result.warnings[0].issue);
